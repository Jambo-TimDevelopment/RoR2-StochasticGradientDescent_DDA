"""
List DDA telemetry sessions from PostHog JSONL exports and check H2/H4 field presence.

Research pipeline: do not filter by survivor (`player_body`). Modded rosters and varied
characters are expected; metrics are aggregated across all sessions unless you add your
own filters outside this tool.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Inspect DDA sessions (all survivors): session_ids by mode and H2/H4 field presence."
    )
    ap.add_argument(
        "inputs",
        nargs="+",
    ...
    session_ids = sorted(set(mode.keys()) | set(body.keys()))
    by_mode = defaultdict(list)
    for sid in session_ids:
        by_mode[mode.get(sid, "")].append(sid)

    print("Sessions by mode (all survivors, no player_body filter):")
